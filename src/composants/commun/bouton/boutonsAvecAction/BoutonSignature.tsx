...
import {
  DocumentsATraiter,
  DocumentsByRequete,
} from "@widget/signature/hook/SignatureDocumentHookUtilDelivrance";
import React, { useCallback, useContext, useEffect, useState } from "react";
import Bouton, { IBoutonProps } from "../Bouton";

interface BoutonSignatureProps extends IBoutonProps {
  libelle: string;
...
interface TableauDataToUse {
  requeteASignerProp: RequeteASigner[];
...
> = ({
  ...
  const handleClickSignature = () => {
    if (
      ...
      if (!requeteASignerProp[0].acte) {
        setRequetesASignerSansActes(
          requeteASignerProp.filter(
            (requeteASigner) =>
              requeteASigner.requete.statut === StatutRequete.A_SIGNER.libelle,
          ),
        );
      } else {
        if (requeteASignerProp.length === 1) {
          validerMentionsPlusieursDocuments(
            () => genererDocuments(requeteASignerProp),
            requeteASignerProp[0].acte,
            requeteASignerProp[0].requete.documentsReponses,
          );
        } else {
          genererDocuments(requeteASignerProp);
        }
  ...
  const signaturePossible = (
    ...
    if (requeteaSignerProp) {
      if (uniqueSignature === true) {
        const requete = requeteaSignerProp[0].requete;
        ...
        estSignaturePossible = requeteaSignerProp.some(
  ...
  const estUnDocumentASigner = (document: IDocumentReponse) => {
  ...
  return (
    <>
      <Bouton
        disabled={
          !signaturePossible(uniqueSignature, requeteASignerProp, connectedUser)
        }
        onClick={() => {
          if (
            !checkDirtyActive ||
            (checkDirtyActive && checkDirty(isDirty, setIsDirty))
          ) {
