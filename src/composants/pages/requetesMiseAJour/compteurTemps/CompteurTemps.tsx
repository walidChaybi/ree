import { CONFIG_GET_DELAI_MISE_A_JOUR_RESTANT } from "@api/configurations/requete/miseAJour/GetDelaiMiseAJourRestant";
...
const CompteurTemps: React.FC<ICompteurTempsProps> = memo(
  ({ idRequete, abandonnerRequete }) => {
    const navigate = useNavigate();
    const [minutesRestantes, setMinutesRestantes] = useState<number | null>(
      null,
    );
    const [prevenirRedirection, setPrevenirRedirection] =
      useState<boolean>(false);
    const { appelApi: getDelaiRestant } = useFetchApi(
      CONFIG_GET_DELAI_MISE_A_JOUR_RESTANT,
    );
