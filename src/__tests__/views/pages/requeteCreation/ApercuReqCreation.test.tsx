...
describe("Test ApercuReqCreation", () => {
  ...
  test("renders VoletPiecesJustificatives Etablissement", async () => {
    ...
    await waitFor(() => screen.getByText("Pièces justificatives"));
  ...
  test.skip("renders VoletPiecesJustificatives Transcription", async () => {
    const router = createTestingRouter(
      [
        {
      ...
      [getUrlWithParam(URL_MES_REQUETES_CREATION_TRANSCRIPTION_APERCU_REQUETE_SIMPLE_ID, "d4f9e898-cf26-42cc-850b-007e9e475e7a")]
    );
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText("Autres pièces justificatives")[0]).toBeDefined();
    });
  });
  ...
  test("Modifier le titre d'un fichier d'une pièce jointe Etablissement", async () => {
    ...
    const inputModificationLibelle = screen.getByLabelText("input-creation-fichierPJ");
  ...
  test("Modifier le titre d'un fichier et revenir en arrière", async () => {
    ...
    const inputModificationLibelle: HTMLInputElement = screen.getByLabelText("input-creation-fichierPJ");
    ...
    await waitFor(() => expect(screen.getAllByText("test libelle")[0]).toBeDefined());

    act(() => fireEvent.click(screen.getAllByTitle("Annuler la modification du libellé")[0]));

    await waitFor(() => expect(screen.getAllByText("Titre de séjour postulant")[0]).toBeDefined());
  });
  ...
  test("Modifier le titre puis annuler", async () => {
    ...
    await waitFor(() => expect(screen.getAllByTitle("Modifier le libellé")[3]).toBeDefined());
  ...
  test("Ouvrir deux pièces jointes côte à côte.", async () => {
    ...
    act(() => fireEvent.click(screen.getByText("Pièces justificatives")));
    ...
    await waitFor(() => {
      ...
      const accordionsAlpha = titreAccordions.filter(accordion => accordion.textContent?.includes("carteIdentite"));
      const accordionsBeta = titreAccordions.filter(accordion => accordion.textContent?.includes("titreSejour"));
