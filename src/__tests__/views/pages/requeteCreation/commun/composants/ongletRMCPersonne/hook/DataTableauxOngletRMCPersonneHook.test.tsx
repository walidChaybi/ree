...
describe("Test le fonctionnement du hook récupérant les données nécessaires pour l'onglet RMC.", () => {
  test("DOIT exécuter une RMC AUTO Personne sur le titulaire de la requête.", () => {
    render(<TestHook />);
  ...
  test("DOIT récupérer les personnes sauvegardees.", () => {
    render(<TestHook />);
  ...
  test("DOIT récupérer les actes ou inscriptions sauvegardes.", () => {
    render(<TestHook />);

    waitFor(() => {
      expect(
        screen.getByTestId("acteInscriptionSelectionne-0").textContent
      ).toBe("c8cbe885-fbd7-4d35-af99-10c827b0238b");
      expect(
        screen.getByTestId("acteInscriptionSelectionne-1").textContent
      ).toBe("747cd416-fcf5-4490-b540-59a89b7f5123");
    });
  });
});
