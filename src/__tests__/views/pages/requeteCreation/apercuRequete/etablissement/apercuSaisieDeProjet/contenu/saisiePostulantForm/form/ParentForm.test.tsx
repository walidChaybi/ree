...
const titulaires = mappingRequeteCreation(requeteCreationEtablissementSaisieProjet).titulaires;
...
describe("Test du bloc Parent de l'onglet Postulant", () => {
  test("DOIT afficher et reseigner les champs des blocs Parent QUAND le formulaire est affiché", () => {
    ...
    const champPrenomParent = screen.getByLabelText("Prénom") as HTMLInputElement;
    const champSexeParent = screen.getByLabelText("Masculin") as HTMLInputElement;
    const champJourNaissanceParent = screen.getByText("Date de naissance").nextElementSibling as HTMLInputElement;
    const champMoisNaissanceParent = champJourNaissanceParent.nextElementSibling?.nextElementSibling as HTMLInputElement;
    const champAnneeNaissanceParent = champMoisNaissanceParent.nextElementSibling?.nextElementSibling as HTMLInputElement;
    const champVilleParent = screen.getByLabelText("Ville") as HTMLInputElement;

    const champEtatRegionParent = screen.getByLabelText("Etat, canton, province") as HTMLInputElement;
