...
describe("Test CourrierFonctions", () => {
  ...
  test("getDefaultValues", () => {
    ...
    expect(getDefaultValuesCourrier(requeteDelivranceInstitutionnel)).toStrictEqual({
      choixCourrier: {
  ...
  test("getTypesCourrier", () => {
    ...
    requete4.choixDelivrance = ChoixDelivrance.REP_SANS_DEL_EC_ACTE_NON_DETENU_AU_SCEC;
  ...
  test("controle formulaire", () => {
    expect(
      controleFormulaire(
        {
          [CHOIX_COURRIER]: {
            [COURRIER]: "062526c5-e5a7-48d1-bc22-11938347f0bc"
          }
        } as SaisieCourrier,
    ...
    expect(
      controleFormulaire(
        {
          [CHOIX_COURRIER]: {
            [COURRIER]: "0296fc7a-fb81-4eb7-a72f-94286b8d8301"
          }
        } as SaisieCourrier,
        [],
        vi.fn()
  ...
  test("getStatutEnTraitement", () => {
    expect(ChoixDelivrance.getStatutApresChoixDelivrance(ChoixDelivrance.DELIVRER_EC_EXTRAIT_AVEC_FILIATION)).toBe(StatutRequete.A_SIGNER);
    expect(ChoixDelivrance.getStatutApresChoixDelivrance(ChoixDelivrance.DELIVRER_EC_COPIE_ARCHIVE)).toBe(StatutRequete.A_VALIDER);
  });
});
