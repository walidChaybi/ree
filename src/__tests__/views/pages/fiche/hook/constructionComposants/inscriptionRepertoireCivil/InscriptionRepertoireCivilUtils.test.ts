...
test("Inscription répertoire civil utils : affichage correct des infos d'une inscription civile  ", () => {
  ...
  const idxMandataire = component.partContent?.contents.findIndex(content => content.libelle === "Mandataire(s)");
  expect(idxMandataire).toBeGreaterThan(-1);
  const valueMandataire: string = component.partContent?.contents[idxMandataire as number].value as string;
  ...
  const valueTypeInscription: JSX.Element = component.partContent?.contents[idxTypeInscription as number].value as JSX.Element;
  ...
  const valueTypeInscription1: JSX.Element = valueTypeInscription.props.children[1][0];
  ...
  const valueTypeInscription2: JSX.Element = valueTypeInscription.props.children[1][1];
  ...
  const idxInscriptionLie = component.partContent?.contents.findIndex(content => content.libelle === "Inscription(s) liée(s)");
  expect(idxInscriptionLie).toBeGreaterThan(-1);
  expect(idxMandataire).toBeLessThan(idxInscriptionLie as number);

  const idxDateInscription = component.partContent?.contents.findIndex(content => content.libelle === "Date d'inscription");
  const valueDateInscription = component.partContent?.contents[idxDateInscription as number].value;
  ...
  const idxDureeInscription = component.partContent?.contents.findIndex(content => content.libelle === "Durée inscription");
  const valueDureeInscription = component.partContent?.contents[idxDureeInscription as number].value;
  ...
  const idxDateFinMEsure = component.partContent?.contents.findIndex(content => content.libelle === "Date fin de mesure");
  const valueDateFinMesure = component.partContent?.contents[idxDateFinMEsure as number].value;
