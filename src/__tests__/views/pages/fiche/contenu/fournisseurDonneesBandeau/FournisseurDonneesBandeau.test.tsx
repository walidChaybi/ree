...
test("Attendu: fournisseur de donnée bandeau pour acte fonctionne correctement", () => {
  ...
  expect(fournisseurDonneesdActe.getSimplePersonnes()[0].prenom).toBe("Paulita");
  expect(fournisseurDonneesdActe.getSimplePersonnes()[1].prenom).toBe("Laurent");
  ...
  expect(fournisseurDonneesdActe.getRegistre()).toBe("CSL.DX.1922.NA.T.410.681");
});
...
test("Attendu: Un acte electronique modifie l'affichage du registre", () => {
  ...
  expect(fournisseurDonneesdActe.getAnnee()).toBe(1921);
  expect(fournisseurDonneesdActe.getRegistre()).toBe("RECE.0000.0000 / CSL.DX.1922.NA.T.410.681");
});
