...
test("le composant DOIT etre rendu correctement QUAND le type et sous-type sont selectionné ", () => {
  ...
  //   expect(screen.getByTestId(inputNiveauDeux)).not.toBeDefined();
  //   expect(screen.getByTestId(inputNiveauTrois)).not.toBeDefined();
...
test("le composant DOIT etre rendu correctement QUAND le type, sous-type et le sous sous-type sont selectionné ", () => {
  render(
    <MemoryRouter>
      <Formik initialValues={{}} onSubmit={() => {}}>
        <ListesTypesMentionForm nom={LISTES_TYPES_MENTION} />
  ...
  //   expect(inputNiveauDeux).not.toBeDefined();
  //   expect(inputNiveauTrois).not.toBeDefined();

  fireEvent.change(screen.getByTestId(inputNiveauUn), {
    target: { value: "0185f3c8-5f4c-4ea9-89e1-fb65fcb7b17f" }
  });

  expect(inputNiveauDeux).toBeDefined();
  //   expect(inputNiveauTrois).not.toBeDefined();
  ...
  expect(inputNiveauTrois).toBeDefined();
