...
describe("Test MiseAJourMentions", () => {
  ...
  const ajouterUneMention = async () => {
    await waitFor(() => expect(screen.getByTestId(LISTE_TYPE_MENTION_NIVEAU_UN)).toBeDefined());
    act(() =>
    ...
    await waitFor(() => expect(screen.getByTestId(LISTE_TYPE_MENTION_NIVEAU_DEUX)).toBeDefined());
    act(() =>
    ...
    await waitFor(() => expect(screen.getByTestId(LISTE_TYPE_MENTION_NIVEAU_TROIS)).toBeDefined());
    act(() =>
    ...
    await waitFor(() => expect(screen.getByPlaceholderText(TEXTE_MENTION_PLACEHOLDER)).toBeDefined());
    act(() =>
    ...
    await waitFor(() => expect(screen.getByText("Ajouter mention")).toBeDefined());
  ...
  test.skip("DOIT afficher le tableau de mentions de l'onglet de Mise A Jour QUAND on ajoute une mention via le formulaire", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTitle("Blablablabla ceci est un texte de mention parfaitement correct")).toBeDefined();
  ...
  test.skip("DOIT desafficher la mention du tableau de mentions de l'onglet de Mise A Jour QUAND on supprime une mention via le formulaire", async () => {
    ...
    await act(async () => await ajouterUneMention());
  ...
  test.skip("DOIT editer le tableau de mentions QUAND on modifie une mention via la modification de mention", async () => {
    const router = createTestingRouter(
      [
        {
          path: URL_REQUETE_MISE_A_JOUR_MENTIONS_SUITE_AVIS_ID,
    ...
    await ajouterUneMention();

    await waitFor(
      () => {
        expect(screen.getByText("Blablablabla ceci est un texte de mention parfaitement correct")).toBeDefined();
        expect(screen.queryByText("Modifier la mention")).toBeDefined();
      },
      { timeout: 5000 }
    );

    // modification de mentions
    act(() => fireEvent.click(screen.queryByText("Modifier la mention") as Element));
    ...
    await waitFor(() => {
      expect(screen.getByTitle("Ceci est une mention de mariage de test.")).toBeDefined();
    });
  ...
  describe("Verrouillage du bouton 'Actualiser et visualiser'", () => {
    test.skip("A l'affichage initial, le bouton est verrouillé", async () => {
      const router = createTestingRouter(
        [
          {
            path: URL_REQUETE_MISE_A_JOUR_MENTIONS_SUITE_AVIS_ID,
