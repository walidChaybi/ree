...
describe.skip("Test generationECHook", () => {
  const ecMariageSansFiliationparams: IGenerationECParams = {
    ...
    requete: getRequeteWithChoixDelivrance(ReponseAppelDetailRequeteDelivrance.data, ChoixDelivrance.DELIVRER_EC_EXTRAIT_SANS_FILIATION),
  ...
  const ecMariageAvecFiliationparams: IGenerationECParams = {
    idActe: idFicheActeMariage,
    requete: getRequeteWithChoixDelivrance(ReponseAppelDetailRequeteDelivrance.data, ChoixDelivrance.DELIVRER_EC_EXTRAIT_AVEC_FILIATION),
    mentionsRetirees: [],
    validation: Validation.N,
  ...
  const ecNaissancePlurilingueparams: IGenerationECParams = {
    idActe: idFicheActe1,
    requete: getRequeteWithChoixDelivrance(ReponseAppelDetailRequeteDelivrance.data, ChoixDelivrance.DELIVRER_EC_EXTRAIT_PLURILINGUE),
    mentionsRetirees: [],
    validation: Validation.N,
  ...
  const ecCopieActeImageMariageparams: IGenerationECParams = {
    idActe: idFicheActeMariage,
    requete: getRequeteWithChoixDelivrance(ReponseAppelDetailRequeteDelivrance.data, ChoixDelivrance.DELIVRER_EC_COPIE_INTEGRALE),
    mentionsRetirees: [],
    validation: Validation.N,
    choixDelivrance: ChoixDelivrance.DELIVRER_EC_COPIE_INTEGRALE
  };

  const HookConsumer: React.FC<IGenerationECParams> = ecMariageFiliationparams => {
  ...
  test("Attendu: un extrait de mariage sans filiation est généré via useGenerationEC", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("bbac2335-562c-4b14-96aa-4386814c02a2")).toBeDefined();
    });
  });
