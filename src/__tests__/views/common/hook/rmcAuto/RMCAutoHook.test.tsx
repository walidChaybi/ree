...
describe("Test RMCAutoHook", () => {
  ...
  const paramsApercuRequete = {
    ...paramsRequete,
    urlCourante: getUrlWithParam(URL_MES_REQUETES_DELIVRANCE_APERCU_REQUETE_ID, "34da88e2-c5c7-4324-ac8e-b35193352e64")
  };
  ...
  const paramsApercuRequeteTraitement: IRMCAutoParams = {
    ...
    urlCourante: getUrlWithParam(URL_MES_REQUETES_DELIVRANCE_APERCU_REQUETE_TRAITEMENT_ID, "34da88e2-c5c7-4324-ac8e-b35193352e64"),
  ...
  test('Test useRMCAutoHook : redirection à partir de "Mes Requêtes"', async () => {
    ...
    await waitFor(() => {
      expect(getByTestId("urlRedirection").textContent).toBe(URL_MES_REQUETES_DELIVRANCE_APERCU_REQUETE_PRISE_EN_CHARGE_ID);
    });
  });
  ...
  test('Test useRMCAutoHook : redirection à partir de "Requêtes Service"', async () => {
    ...
    await waitFor(() => {
      expect(getByTestId("urlRedirection").textContent).toBe(URL_REQUETES_DELIVRANCE_SERVICE_APERCU_REQUETE_PRISE_EN_CHARGE_ID);
    });
  });
  ...
  test('Test useRMCAutoHook : redirection à partir de "Recherche Requêtes"', async () => {
    ...
    await waitFor(() => {
      expect(getByTestId("urlRedirection").textContent).toBe(URL_RECHERCHE_REQUETE_APERCU_REQUETE_PRISE_EN_CHARGE_ID);
    });
  });
