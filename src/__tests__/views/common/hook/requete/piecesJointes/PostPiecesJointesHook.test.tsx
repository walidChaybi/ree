...
const HookConsumerUsePostPiecesJointesApi: React.FC = () => {
  const resultatPostPiecesJointesApi = usePostPiecesJointesApi(TypePieceJointe.PIECE_COMPLEMENT_INFORMATION, "12345", piecesJointesMock);

  return <div>{`${resultatPostPiecesJointesApi?.uuidDocuments[0]}, ${resultatPostPiecesJointesApi?.uuidDocuments[1]}`}</div>;
};
