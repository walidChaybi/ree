...
const afficherComposant = (valeursForm: object, props?: object) =>
  render(
    <Formik initialValues={valeursForm} onSubmit={() => {}}>
      <NomSecableForm {...props} />
    </Formik>
  );
...
describe("Test du composant Nom secable form", () => {
  ...
  test("Affichage du message d'avertissement", () => {
    const { queryByText } = afficherComposant(
      { secable: "true" },
      { afficherAvertissementVocable: true, nomTitulaire: "test test test" }
    );
