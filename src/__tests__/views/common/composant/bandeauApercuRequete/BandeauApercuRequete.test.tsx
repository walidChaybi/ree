...
test("renders titre et numero requete", () => {
  ...
  const titre = screen.getByText(/Requête à signer le 02\/01\/2020 par Ashley YOUNG/i);
  expect(titre.textContent).toBe("Requête à signer le 02/01/2020 par Ashley YOUNG");
});
...
test("récupérer le libellé d'une requête traité à délivrer démat", () => {
  ...
  render(RenderBandeauAvecContexte());
  const element = screen.getByText(/Requête traitée par : Ashley YOUNG - Le : 02\/01\/2020/i);
...
test("récupérer le libellé d'une requête prise en charge et attribuée", () => {
  ...
  render(RenderBandeauAvecContexte());
  screen.getByText(/Requête prise en charge par : Ashley YOUNG - Le : 02\/01\/2020/i);
});

test("récupérer le libellé d'une requête à traiter attribuée", () => {
  requete.statutCourant.statut = StatutRequete.A_TRAITER;
  render(RenderBandeauAvecContexte());
  const element = screen.getByText(/Requête à traiter, attribuée à Ashley YOUNG - Le : 02\/01\/2020/i);
...
test("récupérer le libellé d'une requête en Doublon", () => {
  ...
  render(RenderBandeauAvecContexte());
  const element = screen.getByText(/Requête en doublon, requête déjà en cours avec le N° suivant : 56789 - Le : 01\/01\/2020/i);
  expect(element.className.indexOf("gris") > -1).toBeTruthy();
});
...
test("récupérer le libellé d'une requête a valider", () => {
  ...
  render(RenderBandeauAvecContexte());
  screen.getByText(/Requête à valider, attribuée à Ashley YOUNG - Le : 02\/01\/2020/i);
});

test("récupérer le libellé d'une requête brouillon", () => {
  requete.statutCourant.statut = StatutRequete.BROUILLON;
  render(RenderBandeauAvecContexte());
  screen.getByText(/Requête au statut brouillon initiée par Ashley YOUNG - Le : 02\/01\/2020/i);
});
...
test("récupérer le libellé d'une requête à traiter non attribuée", () => {
  ...
  requete.idUtilisateur = "";
  render(RenderBandeauAvecContexte());
  const element = screen.getByText(/Requête à traiter non attribuée - Créée le 01\/01\/2020/i);
  expect(element.className.indexOf("bleu") > -1).toBeTruthy();
});
