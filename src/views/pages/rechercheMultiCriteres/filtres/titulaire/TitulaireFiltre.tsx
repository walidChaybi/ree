...
const PAYS_NAISSANCE = "paysNaissance";
...
export const TitulaireValidationSchema = Yup.object()
  ...
  .test("prenomInvalide", function (value, error) {
    ...
    const paramsError = {
      ...
      message: "L'astérisque est n'autorisé dans le prénom que si le nom est saisi (avec ou sans *)"
    };

    return prenom != null && prenom?.includes("*") && nom == null ? this.createError(paramsError) : true;
  });
...
const TitulaireFiltre: React.FC<TitulaireFiltreProps> = props => {
  ...
  function onClickRappelCriteresTitulaire(event: React.MouseEvent, titulaire: ITitulaireRequete): void {
    ...
    if (titulaire.nomNaissance) {
      props.formik.setFieldValue(withNamespace(props.nomFiltre, NOM), titulaire.nomNaissance);
    }
    ...
    if (titulaire.paysNaissance) {
      props.formik.setFieldValue(withNamespace(props.nomFiltre, PAYS_NAISSANCE), titulaire.paysNaissance);
    }
    if (titulaire.jourNaissance) {
      props.formik.setFieldValue(withNamespace(dateNaissanceNamespace, JOUR), rempliAGaucheAvecZero(titulaire.jourNaissance));
    }
    if (titulaire.moisNaissance) {
      props.formik.setFieldValue(withNamespace(dateNaissanceNamespace, MOIS), rempliAGaucheAvecZero(titulaire.moisNaissance));
    }
  ...
  return (
    <div className={props.nomFiltre}>
      <Fieldset titre={"Filtre titulaire"}>
