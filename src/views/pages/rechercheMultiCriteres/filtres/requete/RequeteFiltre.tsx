...
import { ComponentFiltreProps, FormikComponentProps, onBlurChampNumero, withNamespace } from "@widget/formulaire/utils/FormUtil";
...
// Noms des champs
const NUMERO_REQUETE = "numeroRequete";
const NUMERO_TELEDOSSIER = "numeroTeledossier";
const NUMERO_DOSSIER_NATIONAL = "numeroDossierNational";
const TYPE_REQUETE = "typeRequete";
const SOUS_TYPE_REQUETE = "sousTypeRequete";
const STATUT_REQUETE = "statutRequete";
...
export const RequeteValidationSchema = Yup.object({
  [NUMERO_REQUETE]: Yup.string().matches(CaracteresAlphanumeriques, CARACTERES_ALPHANUMERIQUE),
...
const RequeteFiltre: React.FC<RequeteFiltreProps> = props => {
  const numeroRequeteWithNamespace = withNamespace(props.nomFiltre, NUMERO_REQUETE);
  const numeroTeledossierWithNamespace = withNamespace(props.nomFiltre, NUMERO_TELEDOSSIER);
  const numeroSDANFWithNamespace = withNamespace(props.nomFiltre, NUMERO_DOSSIER_NATIONAL);
  const typeRequeteWithNamespace = withNamespace(props.nomFiltre, TYPE_REQUETE);
  const sousTypeRequeteWithNamespace = withNamespace(props.nomFiltre, SOUS_TYPE_REQUETE);
  const statutRequeteWithNamespace = withNamespace(props.nomFiltre, STATUT_REQUETE);

  const [sousTypeRequeteInactif, setSousTypeRequeteInactif] = useState<boolean>(true);

  const [statutRequeteInactif, setStatutRequeteInactif] = useState<boolean>(true);
  ...
  const gestionTypeRequeteOptions = (type: string) => {
    if (type) {
      setSousTypeRequeteOptions(SousTypeRequeteUtil.getOptionsAPartirTypeRequete(TypeRequete.getEnumFor(type)));
    }
  };

  const gestionStatutRequeteOptions = (type: string) => {
    if (type) {
      setStatutRequeteOptions(StatutRequete.getOptionsAPartirTypeRequete(TypeRequete.getEnumFor(type)));
    }
  };
