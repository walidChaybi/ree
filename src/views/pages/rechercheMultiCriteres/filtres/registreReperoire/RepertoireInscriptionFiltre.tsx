...
export type RepertoireInscriptionFiltreProps = TypeRepertoireInputProps & ComponentFiltreProps & FormikComponentProps;

const RepertoireInscriptionFiltre: React.FC<RepertoireInscriptionFiltreProps> = props => {
  const numeroInscriptionWithNamespace = withNamespace(props.nomFiltre, NUMERO_INSCRIPTION);
  const typeRepertoireWithNamespace = withNamespace(props.nomFiltre, TYPE_REPERTOIRE);
  const natureInscriptionWithNamespace = withNamespace(props.nomFiltre, NATURE_INSCRIPTION);
  ...
  const manageNatureOptions = async (type: string) => {
    if (type === "RC") {
      setNatureOptions(NatureRc.versOptions());
    } else if (type === "RCA") {
      setNatureOptions(NatureRca.versOptions());
    } else {
      setNatureOptions([]);
    }
  };
  ...
  const formatNumber = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length === limitChar + 1 && value.charAt(limitChar) !== "-" && !value.includes("-")) {
  ...
  useEffect(() => {
    ...
    const type = getIn(props.formik.values, withNamespace(props.nomFiltre, TYPE_REPERTOIRE));
      ...
      <div className="FormFiltre">
        <SelectField
          ...
          options={TypeRepertoire.getAllEnumsAsOptions().filter(el => {
            return props.filtreTypeRepertoire ? el.cle === props.filtreTypeRepertoire.libelle : true;
          })}
