...
type FrancisationPostulantFormProps = IFrancisationPostulantFormProps & FormikComponentProps;

const FrancisationPostulantForm: React.FC<FrancisationPostulantFormProps> = props => {
  const nomChampNom = withNamespace(props.nom, NOM);
  const nomChampPrenom = withNamespace(props.nom, PRENOMS);

  const estAfficheChampNom = Boolean(props.formik.getFieldProps(nomChampNom).value);
  const estAfficheChampPrenom = filtrePrenomsFrancises(props.retenueSdanf?.prenomsRetenu).length > ZERO;

  return (
    <Item
      titre={"Francisation postulant"}
...
export default connect<IFrancisationPostulantFormProps>(FrancisationPostulantForm);
