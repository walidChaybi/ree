...
import { EvenementFormDefaultValues, EvenementFormValidationSchema } from "./sousFormulaires/evenement/EvenementForm";
import { IdentiteFormDefaultValues, IdentiteFormValidationSchema } from "./sousFormulaires/identite/IdentiteForm";
import { LienTitulaireFormDefaultValues, LienTitulaireFormValidationSchema } from "./sousFormulaires/lienTitulaire/LienTitulaireForm";
import { MandantFormDefaultValues, MandantFormValidationSchema } from "./sousFormulaires/mandant/MandantForm";
import { RequerantFormValidationSchema, TitulairesFormDefaultValues } from "./sousFormulaires/requerant/RequerantForm";
...
export const SaisirRDCPage: React.FC = () => {
  ...
  const [lienTitulaireVisible, setLienTitulaireVisible] = useState<boolean>(true);
  const [optionsRequerant, setOptionsRequerant] = useState<Options>(TypeRequerantRDC.getListEnumsAsOptions(UN_TITULAIRE));
  ...
  useEffect(() => {
    if (detailRequeteState) {
      const requeteForm = mappingRequeteDelivranceVersFormulaireRDC(detailRequeteState as IRequeteDelivrance);
  ...
  useEffect(() => {
    if (requeteRDCResultat?.requete) {
      ...
      const pjAMettreAjour = getPiecesJointesNonVides(requete?.[PIECES_JOINTES]);
  ...
  const onChangeRequerant = (typeRequerant: string) => {
    modificationChamps(typeRequerant, setMandantVisible, setLienTitulaireVisible, setOptionsLienTitulaire);
  };
