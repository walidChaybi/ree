...
export class GestionnaireRenumerotationMentions {
  public renumerotationMentions(
    ...
    res = res.filter(
      ...
      mention =>
        !(
          !mentionsSurEcran.find(MsE => MsE.id === mention.id) &&
          this.contientTexteLieAuDocument(mention, typeDocument)
        )
    );
  ...
  private contientTexteLieAuDocument(
    mention: MentionPourRenumerotation,
    typeDocument: string
  ) {
    return (
      (mention.textes.texteMentionDelivrance &&
        DocumentDelivrance.estExtraitAvecOuSansFilliation(typeDocument)) ||
      (mention.textes.texteMentionPlurilingue &&
        DocumentDelivrance.estExtraitPlurilingue(typeDocument))
    );
  }

  private remplacerNumeroOrdreExtrait(
    mentionsEnBase: MentionPourRenumerotation[]
  ) {
    if (mentionsEnBase.some(mention => !mention.numeroOrdreExtrait)) {
      mentionsEnBase.forEach(
        mention => (mention.numeroOrdreExtrait = mention.numeroOrdre)
      );
    }
  }

  private numerotationMentionsEnBase(
    mentionsSurEcran: IMentionAffichage[],
    mentionsEnBase: MentionPourRenumerotation[]
  ) {
    const mentionsEnBaseSurEcran = mentionsSurEcran.filter(mention =>
    ...
    mentionsEnBaseSurEcran.forEach((mention, index) => {
      ...
      const indexMentionCourante = mentionsEnBase.findIndex(
        el => el.id === mention.id
      );
      if (mentionsEnBaseSurEcran[0].id === mention.id) {
        this.numerotationMentionsEnBaseSiPremiere(
          mentionsSurEcran,
          mentionsEnBase,
          indexMentionCourante
        );
      } else {
        this.numerotationMentionsEnBaseSiPasPremiere(
          mentionsSurEcran,
          mentionsEnBase,
          indexMentionCourante,
          mentionsEnBaseSurEcran,
          index
        );
      }
    });
  ...
  private numerotationMentionsEnBaseSiPremiere(
    ...
    while (i < indexMentionCourante) {
      if (
        !mentionsSurEcran
          .map(mentionMap => mentionMap.id)
          .includes(mentionsEnBase[i].id)
      ) {
  ...
  private numerotationMentionsEnBaseSiPasPremiere(
    ...
    let i = mentionsEnBase.findIndex(
      el => el.id === mentionsEnBaseSurEcran[index - 1].id
    );
    ...
    if (
      mentionsEnBase[i].id ===
      mentionsEnBaseSurEcran[mentionsEnBaseSurEcran.length - 1].id
    ) {
      i = 0;
      c++;
...
export const gestionnaireRenumerotationMentions =
  new GestionnaireRenumerotationMentions();
