...
export const mappingVersMentionsApi = (mentionsApi: IMention[], mentionsAffichage: IMentionAffichage[], typeDocument: string) => {
  ...
  const mentionsRenumerote = gestionnaireRenumerotationMentions.renumerotationMentions(mentionsAffichage, mentionsApi, typeDocument);
  ...
  mentionsRenumerote.forEach(mR => {
    ...
    const mentionAAjouter = {
      ...
      typeMention: {
        ...
        idTypeMention: TypeMention.getIdTypeMentionDepuisIdNature(mR.typeMention["idNatureMention" as keyof ITypeMention] as string)
      },
      id: mR.id
    };
    if (mention) {
...
export const modificationEffectue = (
  ...
  if (mentions && mentionsApi && document) {
    if (!shallowEgalTableau(mentions, mappingVersMentionAffichage(mentionsApi, document, natureActe))) {
      return true;
    } else {
      return false;
    }
...
export const mappingVersMentionAffichage = (
  ...
  if (DocumentDelivrance.estCopieIntegrale(document.typeDocument)) {
    mentionsPourAffichage = mappingVersMentionAffichagePourCopieIntegrale(mentionsApi, document);
  } else if (DocumentDelivrance.estExtraitAvecOuSansFilliation(document.typeDocument)) {
    mentionsPourAffichage = mappingVersMentionAffichagePourExtraitAvecOuSansFiliation(mentionsApi, document);
  } else if (DocumentDelivrance.estExtraitPlurilingue(document.typeDocument)) {
    mentionsPourAffichage = mappingVersMentionAffichagePourExtraitPlurilingue(mentionsApi, document, natureActe);
  }
...
export const mappingVersMentionAffichagePourCopieIntegrale = (mentionsApi: IMention[], document: IDocumentReponse): IMentionAffichage[] => {
...
export const mappingVersMentionAffichagePourMiseAJour = (mentions: IMention[]): IMentionAffichage[] => {
  return mentions.reduce<IMentionAffichage[]>((mentionsSelectionnees, mention) => {
