...
export const AnalyseMarginale = {
  getTitulairesDansLOrdre(am?: IAnalyseMarginale): ITitulairesAM {
    ...
    if (am) {
      const titulaires = triListeObjetsSurPropriete([...am.titulaires], "ordre");
   ...
   */
  getAnalyseMarginaleLaPlusRecente(analyseMarginales?: IAnalyseMarginale[]): IAnalyseMarginale | undefined {
    let analyseMarginaleLaPlusRecente: IAnalyseMarginale | undefined;

    if (analyseMarginales && analyseMarginales.length > 0) {
      const analyseMarginalesTries = triListeObjetsSurDate([...analyseMarginales], "dateDebut");

      let idx = analyseMarginalesTries.length - 1;
      analyseMarginaleLaPlusRecente = analyseMarginalesTries[idx];
      while (analyseMarginaleLaPlusRecente?.dateFin && DateUtils.estDateAtteinte(analyseMarginaleLaPlusRecente?.dateFin)) {
        idx--;
        analyseMarginaleLaPlusRecente = analyseMarginalesTries[idx];
      }
      ...
      if (!analyseMarginaleLaPlusRecente) {
        analyseMarginaleLaPlusRecente = analyseMarginalesTries[analyseMarginalesTries.length - 1];
      }
    }
